...
import ru.netology.data.info.CreditRequestInfo;
import ru.netology.data.info.OrderInfo;
import ru.netology.data.info.PaymentInfo;
...
public class DataHelper {
    ...
    @SneakyThrows
    public static CreditRequestInfo getCreditRequestInfo() {
        var runner = new QueryRunner();
        var creditRequestInfo = "SELECT * FROM credit_request_entity WHERE created = (SELECT MAX(created) FROM credit_request_entity);";

        try (var connection = DriverManager.getConnection(
                datasource, "app", "pass")) {
            return runner.query(connection, creditRequestInfo, new BeanHandler<>(CreditRequestInfo.class));
        }
    }
